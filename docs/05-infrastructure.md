...
### 音源の構成：音声ファイル + 手続き生成の床

ゴースト 1 体につき 2 つの音を**同じ座標**から鳴らす。

| 音 | 役割 |
| --- | --- |
| 音声ファイル（`public/audio/ghost-voice.mp3`、任意） | 定期的に鳴る。存在を強く印象づける |
| 手続き生成のノイズ | その下に薄く常時鳴る。方向を継続的に掴む床になる |

同じ位置から鳴るため、方向が混線することはない。

**音声ファイルはリポジトリに含めていない。**
置かれていない場合は手続き生成のみで成立する（フォールバック）。
権利関係の都合で素材を同梱せず、各自が用意する運用にしている。

### 音声ファイルの扱い

**`PannerNode` に渡す前に必ずモノラルへ落とす。**
ステレオのまま渡すと、ファイルが持つ左右の音像と HRTF の定位が干渉し、
方向が曖昧になる。

```typescript
// infrastructure/audio/VoiceSample.ts
...
MP3 のデコードは `decodeAudioData` が標準で対応するため、
ビルド時の音声変換ツール（ffmpeg 等）は不要である。

#### 間（gap）を入れる理由

素材が一続きの発話やジングルである場合、シームレスにループさせると
同じ台詞が途切れなく鳴り続け、恐怖ではなく滑稽になる。
クリップの後ろに無音を連結したバッファをループさせることで、
「鳴る → 間 → また鳴る」という繰り返しになる。

無音をバッファに含める方式を採るのは、`setTimeout` による再生管理だと
タイマーの誤差が蓄積し、タブが非アクティブな間に間隔が崩れるためである。

#### 素材の用意と差し替え

`public/audio/ghost-voice.mp3` を置くだけでよい。
ステレオ・モノラルのどちらでも受け付ける（読み込み時にモノラル化される）。
ファイルが無い場合は手続き生成の音のみで動作する。

| 項目 | 指針 |
| --- | --- |
| 形式 | MP3（`decodeAudioData` が対応する形式なら可） |
| チャンネル | どちらでも可。モノラルならファイルサイズが半分で済む |
| 長さ | 数秒〜十数秒。長いほど繰り返しの間隔が開く |
| 音量 | ピークが 0 dBFS 近くまで正規化されていること。距離減衰はコード側で行う |
