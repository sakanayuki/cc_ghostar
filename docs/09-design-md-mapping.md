...
| §10 | Virtual World | [03.1](./03-domain-model.md#31-座標系)、[04.7](./04-game-rules.md#47-ゴーストの初期配置) | 踏襲。方位の符号と分離制約を追加 |
...
| 14 | プレイ構成 | 固定 3 体・同時出現 | §10 に 3 体の例示のみ |
...
| **棄却サンプリングが成立しなかった** | 最小分離角 100° では実行可能領域が狭く、試行が頻繁に失敗してフォールバックへ落ち、そのフォールバックが分離制約を破っていた。隙間そのものを構成する方式へ変更 | [04.7](./04-game-rules.md#47-ゴーストの初期配置) |
