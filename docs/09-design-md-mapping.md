...
## 9.4.5 原典 §36「初期に実装しない」との関係

原典 §36 は Multiplayer と Backend を除外項目として挙げていた。
マルチプレイ（[10](./10-multiplayer.md)）の追加はこれに抵触する。

ただし除外の意図は「PoC の初期スコープを絞ること」であり、
PoC が成立した後の拡張まで禁じるものではないと解釈した。
また **Backend は追加していない**。サーバーを立てず、端末どうしの
直接接続と、既存の公開シグナリングサーバーだけで構成している。
...
| `fetch` / `XMLHttpRequest` を使用しない | プライバシー方針（原典 §31）の機械的強制。同梱アセットの読み込みのみ個別に解除する |
| 受信したネットワークデータは必ず検証してから使う | 回線の向こうから来る値は型で守れない（[10.4](./10-multiplayer.md#104-受信データの検証)） |
| カメラ映像は通信に含めない | 映像を端末の外へ出さないという方針は、マルチプレイ追加後も維持する |
...
### 音声ファイル読み込みの追加と、プライバシー規則の見直し

ゴーストの声に**音声ファイルを使える経路**を追加した際、
自分で課した ESLint 規則（`fetch` の全面禁止）が**自前アセットの読み込みまで
弾いている**ことが判明した。

規則の意図は「外部への送信をしない」（[01.5](./01-overview.md#15-プライバシー方針)）で
あって、同一オリジンの同梱アセットを読むことではない。規則が意図より広すぎた。

そこで規則自体は残したまま、同梱アセットを読む箇所だけを
理由コメント付きの `eslint-disable-next-line` で個別に解除する運用に変えた。
外部送信を禁じる本来の意図は維持される。

あわせて、検証に使った素材の性質と、それに対する設計。
（素材自体はテスト用だったためリポジトリには含めていない。）

| 判明したこと | 対応 |
| --- | --- |
| 素材の L/R 相関が 1.00000（実質デュアルモノ） | モノラル化による損失がない。実測の最大差 2.81e-5 |
| 10.6 秒の一続きの発話で、ループ素材ではない | シームレスループをやめ、末尾に無音を連結したバッファをループさせる方式に |
| MP3 のループは継ぎ目で不連続が出やすい | 無音区間をバッファに含めることで継ぎ目が無音になり、問題が起きない |
| 音声だけだと鳴っていない間の方向が分からない | 手続き生成のノイズを同じ座標から薄く鳴らし続ける「床」として残した |
