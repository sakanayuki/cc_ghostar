...
| 概念 | 値 | 用途 |
| --- | --- | --- |
| カメラ FOV（垂直） | 60°（原典 §17） | 描画。「見えるかどうか」を決める |
| **カメラ FOV（水平）** | **約 30°**（縦持ち 390×844 の場合） | 実際に左右で見えている範囲 |
| 光錐の半頂角 | 8°（[04.4](./04-game-rules.md#44-チューニング値gameconfig)） | 判定。「捕捉できるかどうか」を決める |

### 縦持ちでは水平視野が制約になる

**`PerspectiveCamera` の FOV は垂直方向の値である。**縦持ちの画面はアスペクト比が
1 より小さいため、水平方向の視野は垂直よりはるかに狭くなる。

```text
垂直半視野   = 30°
水平半視野   = atan( tan(30°) × 390/844 ) ≈ 14.9°
```

したがって**光錐の半頂角を 15° より広く取ると、画面の外にいるゴーストを
捕捉できてしまう**。これは「狙って光を当てる」というコアメカニクスを
無効化するため、`beamHalfAngle` は水平半視野より明確に小さい 8° とする。

> この問題は実装して実機相当の画面比で動かすまで顕在化しなかった。
> 垂直 FOV の数値だけを見て「60° の視野に対して 20° の光錐は十分狭い」と
> 判断したことが原因である。HUD の照準円（[06.4](./06-ui-ux.md#64-hud-の構成)）は
> この関係を可視化するため、設定を誤れば画面からはみ出して即座に気づける。

光錐を視野より明確に狭くする理由は 2 つある。
